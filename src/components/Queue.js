...
const Queue = ( props ) => {
    return props.queue.map( ( order, index ) => {
        return (
            <div className="order-card card card-primary mb-2">
                <div className="card-body">
                    ...
                    <button type="button" className="btn btn-sm btn-link p-0" onClick={ () => props.deleteOrder( order.orderId ) }>{ `Удалить` }</button>
                </div>
            </div>
        );
    } );
